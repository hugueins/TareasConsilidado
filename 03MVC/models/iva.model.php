...
class iva
{
    ...
    public function insertar($Detalle, $Estado, $Valor) //insert into iva (Detalle, Estado, Valor) values ($nombre, $Estado, $Valor)
    {
        try {
            $con = new ClaseConectar();
            $con = $con->ProcedimientoParaConectar();
    ...
    public function actualizar($idIVA, $Detalle, $Estado, $Valor) //update iva set nombre = $nombre, Estado = $Estado, Valor = $Valor where id = $id
    {
        try {
            $con = new ClaseConectar();
            $con = $con->ProcedimientoParaConectar();
            $cadena = "UPDATE `iva` SET `Detalle`='$Detalle',`Estado`=$Estado,`Valor`=$Valor,`Con WHERE `idIVA` = $idIVA";
            if (mysqli_query($con, $cadena)) {
                return $idIVA;
            } else {
                return $con->error;
